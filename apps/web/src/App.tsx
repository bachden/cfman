...
export default function App() {
  ...
  return <AppShell username={data.user.username}><DrawerProvider><PublicBaseUrlBanner />{data.user.mustChangePassword && <button className="password-banner" onClick={() => navigate("/settings")}><AlertTriangle size={16} /><span>The default password is still active.</span><strong>Change password</strong></button>}<Suspense fallback={<div className="app-loading"><div className="brand-spinner" /><span>Loading script editor...</span></div>}><Routes><Route path="/" element={<DashboardPage />} /><Route path="/accounts" element={<AccountsPage />} /><Route path="/tunnels" element={<TunnelsPage />} /><Route path="/scripts" element={<ScriptsPage />} /><Route path="/audit" element={<AuditPage />} /><Route path="/settings" element={<SettingsPage user={data.user} onLogout={() => queryClient.removeQueries({ queryKey: ["auth", "me"] })} onPasswordChanged={updatePasswordState} />} /><Route path="*" element={<Navigate to="/" replace />} /></Routes></Suspense></DrawerProvider></AppShell>;
}
