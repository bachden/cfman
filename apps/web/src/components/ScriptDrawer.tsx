...
export function ScriptDrawer({ scriptId, version, initialBulkRunId, openSeq, onClose, width, onResize }: { scriptId: string | null; version: number | null; initialBulkRunId?: string | null; openSeq: number; onClose: () => void; width: number; onResize: (width: number) => void }) {
  const queryClient = useQueryClient();
  const { openTunnelDrawer } = useDrawers();
  const [bulkDetailWidth, setBulkDetailWidth] = useState(width);
  ...
  return <>
    <SideDrawer open={Boolean(scriptId) && !initialBulkRunId} focusKey={String(openSeq)} width={width} onResize={onResize} title={<div className="drawer-heading">{detail ? <HostPlatformIcon platform={detail.platform} size={18} /> : null}<strong>{detail?.name ?? "Script details"}</strong></div>} onClose={onClose}>
    ...
    <SideDrawer open={Boolean(bulkDetailRun)} focusKey={bulkDetailRun?.id ?? null} width={bulkDetailWidth} onResize={setBulkDetailWidth} title={<div className="drawer-heading"><Layers3 size={18} /><strong>{bulkDetailRun?.name ?? "Bulk execution"}</strong></div>} onClose={() => { const wasDeepLinked = Boolean(initialBulkRunId) && bulkDetailRun?.id === initialBulkRunId; setBulkDetailRun(null); if (wasDeepLinked) onClose(); }}>
      {bulkDetailRun && <div className="bulk-execution-detail"><header><div><p>{bulkDetailRun.description || "No description"}</p></div><time>{new Date(bulkDetailRun.createdAt).toLocaleString()}</time></header>{(() => {
        ...
        <div className="script-execution-list">{bulkDetailData?.executions.map((execution) => {
          ...
          const isAi = execution.requestedVia === "mcp";
          return <details className={`command-execution command-execution-${execution.status}`} key={execution.id} open={isOpen} onToggle={(event) => { if (event.currentTarget.open) setExpandedBulkExecutionId(execution.id); else if (expandedBulkExecutionId === execution.id) setExpandedBulkExecutionId(null); }}><summary><span className="command-execution-summary-main"><StatusBadge status={execution.status} />{isAi && <span className="command-execution-source-tag">AI</span>}<button className="script-execution-tunnel-link" type="button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); openTunnelDrawer(execution.tunnelId, "connect"); }}>{execution.tunnelDisplayName}</button><code className="command-execution-tunnel-code">{execution.tenantCode} / {execution.tunnelCode}</code><span className="host-identity" title={environment}><HostPlatformIcon environment={execution.environment} platform={execution.enrollmentPlatform} osName={execution.osName} /><code>{execution.computerName ?? "N/A"}</code></span></span><span className="command-execution-timing"><time>{new Date(execution.startedAt ?? execution.createdAt).toLocaleString()}</time><code>{execution.elapsedMs === null ? execution.status : `${execution.elapsedMs} ms`}</code></span></summary>{isOpen && <div className="command-execution-body"><ExecutionLog tunnelId={execution.tunnelId} execution={execution} /></div>}</details>;
