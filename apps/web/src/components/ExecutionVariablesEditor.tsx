...
export function ScriptArgumentsEditor({ argumentsList, onChange, description }: { argumentsList: ScriptArgument[]; onChange: (argumentsList: ScriptArgument[]) => void; description?: string }) {
  ...
  return <section className="script-arguments-editor">
    ...
    {argumentsList.length ? <div className="script-argument-list">{argumentsList.map((argument, index) => {
      ...
      // $NAME in a default value always resolves against the tunnel's variables.
      // If another argument carries that same name, the operator most likely
      // expected the argument's value, so point the ambiguity out.
      const argumentNames = new Set(argumentsList.map((item) => item.name.toUpperCase()));
      const clashingRefs = referencedVariableNames(argument.defaultValue).filter((name) => argumentNames.has(name));
      if (!editingIndexes.has(index)) return <div className="script-argument-row script-argument-row-display" key={index}>
        ...
        <span className="script-argument-value-label">{argument.defaultValue || "—"}{clashingRefs.length > 0 && <span className="script-argument-shadow-flag" title={`$${clashingRefs[0]} resolves to the VARIABLE ${clashingRefs[0]}, not to the argument of the same name. An argument name is not a variable name - keep them distinct to avoid confusion.`}>name clash</span>}</span>
      ...
      return <div className="script-argument-row" key={index} onKeyDown={(event) => { if (event.key === "Escape") { event.stopPropagation(); cancelEditing(index); } }}>
        ...
        <label className="field"><span className="field-label">Default value{clashingRefs.length > 0 && <span className="script-argument-shadow-flag" title={`$${clashingRefs[0]} resolves to the VARIABLE ${clashingRefs[0]}, not to the argument of the same name. An argument name is not a variable name - keep them distinct to avoid confusion.`}>name clash</span>}</span><input className={clashingRefs.length ? "input-warning" : undefined} value={argument.defaultValue} onChange={(event) => update(index, { defaultValue: event.target.value })} /></label>
